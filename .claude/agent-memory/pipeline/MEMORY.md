...
- [Search pipeline (Wave 1)](search-pipeline.md) — second pipeline kind on the same engine; SearchPipeline facade + CollectionReadPort via the DEAD bind() seam; 3 gotchas: bind() never called by engine, run-inputs must be Artifacts, public_models can't subclass ScoredOutput (layering).
- [Shared-family palette scoping](shared-family-palette-scoping.md) — `deliver` is shared by ingest/search; each facade scopes its palette via a `FAMILY_KINDS` allowlist passed to `FamilyCatalog.from_family(family, kinds)`; add any newly-shared family there.
