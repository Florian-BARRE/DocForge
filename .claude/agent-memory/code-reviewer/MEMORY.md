...
- [Secondary step fails ingest](antipattern-secondary-step-fails-ingest.md) — a post-index() denormalization/secondary-store step inside the fatal ingest try can mark a fully-ingested doc FAILED + force a full re-embed on retry; make it best-effort with a backfill repair path
