...
| `WORKER_OVER_JOB_TIMEOUT_GRACE_SECONDS` | `300.0` | Job-level watchdog grace. The reaper's **second, heartbeat-independent** condition fails a RUNNING job **on a live worker** whose total age exceeds its effective job timeout (per-collection `job_timeout_seconds`, else `WORKER_JOB_TIMEOUT_SECONDS`) **plus this grace** — the backstop for a wedged native stage arq's async cancel cannot kill, which a fresh heartbeat would otherwise protect forever (`job_timeout_exceeded`). Generous so a job the engine is about to cancel cleanly at its job timeout is never falsely reaped; it never shortens a job's real job timeout. |
