...
flowchart LR
    ...
    subgraph S2["Étape 2 — PARSE ✅"]
        s2["docling (défaut) → granite_docling / pp_structure / paddleocr_vl (escalade)"]
    end
...
├── parse/          parser/docling · parser/granite_docling · parser/pp_structure · parser/paddleocr_vl · figure_render
...
- **`UNIQUE_IN_GRAPH`** (le flag de multiplicité, exposé par `describe()` et rejeté par le validateur en
  ...
  pp_structure · paddleocr_vl — chacun unique par kind, mais empilables en escalade car kinds DIFFÉRENTS · figure_render ·
...
- **Trace d'exécution** (le `NodeExecutionRecord` par node) : le moteur émet un record par node (même en succès), imbriqué en arbre (un groupe niche ses enfants, un item de ForEach devient `body[i]`). Chaque record **remonte le `score`** d'un node scoré exactement comme il remonte l'`usage` — lu sur la sortie (`getattr(output, "score", None)`), donc **survit à `TraceLevel.OFF`** (aucun payload capturé) et se généralise à TOUTE feuille scorée (parser docling/granite/pp_structure, ocr…), pas seulement docling. **Capture des payloads — 3 niveaux (`TraceLevel`, ordre `off < shape < full`)** : `off` ne capture rien (le runner SEARCH, qui jette le record) ; `shape` (défaut INGEST) attache un **descripteur de forme** peu coûteux et SUPERFICIEL (`RecordTrace.summarize` → `{type, fields, sizes, item_count, hash}`, jamais de `model_dump` profond de l'IR) inline dans `input_summary`/`output_summary` — aucun contenu brut au repos ; `full` (opt-in par collection via `collection.trace_verbosity`, borné par le plafond opérateur `WORKER_TRACE_MAX_VERBOSITY`) ajoute le **payload brut strippé** écrit dans SeaweedFS par content-hash (clé `trace/{job_id}/{hash}`, dédup un IR inchangé entre hops, préfixe-job pour le GC) — la ligne garde seulement `input_ref`/`output_ref` + `has_full_input`/`has_full_output`, tronqué au-delà de `WORKER_TRACE_PAYLOAD_MAX_BYTES`. La capture est **best-effort** : une erreur de résumé/store est loggée et abandonnée, jamais un échec d'ingestion. **Après le run, le worker persiste l'ARBRE COMPLET** sur la timeline `job_stage_event` (façade `persist_execution_tree`, modèle **materialized-path** : `node_path` racine = id nu, imbriqué = chemin pointé `enrich.figures[3].vlm` · `depth` · `parent_path` · `item_index` · `score`) : les lignes racines ouvertes en live (une par stage) sont complétées en place, chaque node **imbriqué** (enfant de groupe, instance d'item ForEach) est inséré. Exposé plat sur `GET /jobs/{id}/events` (l'UI reconstruit l'arbre depuis `node_path`/`depth` ; ordre pré-ordre parent-avant-enfant, items ForEach par index croissant). Un run ÉCHOUÉ persiste l'arbre PARTIEL lui aussi : le moteur retourne l'arbre construit jusqu'au node fautif (feuille `status=error`), le runner le CARRY sur `PipelineRunError.record`, et le worker le persiste (best-effort, avant le `mark_failed` — un échec de persist ne masque jamais l'erreur réelle du job) → la trace d'un job FAILED montre le même arbre imbriqué jusqu'au node en échec, réconcilié par `(job_id, node_path)` comme le succès. Un échec trop précoce (graphe invalide, preflight, rehydrate) n'a pas de record → seules les lignes racines live subsistent, comme avant.
