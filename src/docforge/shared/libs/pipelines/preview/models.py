# ====== Code Summary ======
# The pipeline-preview response models — the bounded dry-run report: an IR summary, the first N
# chunks (text truncated), the run's ACTUAL metered cost, the full execution trace, and any warnings.
# Every field is a projection of the in-memory RunBundle + NodeExecutionRecord the inline run produced
# (see PreviewProjector); nothing here is ever persisted. Lives in the lib (not a router models file)
# so the service/projector builds it without importing a router — the same placement as CostEstimate.
