# ====== Code Summary ======
# Translates a MinerU2.5-Pro sidecar response into the canonical DocumentIR — the mineru→IR adapter.
# The MinerU sidecar (src/mineru_server, POST /parse) emits the SAME clean per-page block contract the
# PP-StructureV3 / PaddleOCR-VL sidecars do (`{pages:[{page_index, image_width, image_height,
...
# duplicating them. The MinerU sidecar already resolved MinerU's `type`/`text_level` into the
# pp_structure-style labels (title/paragraph_title/subsection preserve H1/H2/H3, table→html,
# equation→formula latex, image/chart→figure) and emitted bboxes in a 0–1000 space with
# image_width/height=1000, so make_provenance's divide-by-dims yields [0, 1] unchanged. Only the class
# identity differs (a distinct brick in the palette / trace); the per-block logic is the shared logic.
# Pure module — no engine import, so it maps a canned response in a unit test without a live sidecar.
...
class MineruIRMapper:
    """Static adapter mapping a MinerU2.5-Pro sidecar response into the canonical DocumentIR.

    Consumes the SAME sidecar contract as PpStructureIRMapper and reuses its helpers/flattener; kept a
    distinct class so mineru is its own brick in the palette and execution trace.
    """

    logger = loggerplusplus.bind(identifier="MineruIRMapper")

    def __new__(cls, *args: object, **kwargs: object) -> None:
        raise TypeError("MineruIRMapper is a static-only class and cannot be instantiated.")
    ...
    def __map_block(
        ...
        # 1. Resolve the label → BlockType, or skip the block entirely (unmapped labels).
        label = str(raw.get("label", "")).strip()
        block_type = PpStructureParseHelpers.label_to_block_type(label)
        if block_type is None:
            return None

        # 2. Normalize the pixel bbox against the page-image dims (top-left origin, no y-flip). The
        #    MinerU sidecar emits a 0–1000 space with dims=1000, so this yields [0, 1] directly.
    ...
    def map_response(cls, response: dict[str, Any], doc_id: str, source_hash: str) -> DocumentIR:
        """
        Translate a MinerU2.5-Pro sidecar response into a canonical DocumentIR.
        ...
        #    assigns it from the content_list's already-visual order). The IR's reading_order is a
        ...
        cls.logger.debug(f"MinerU→IR: {doc_id} → {n_pages} pages, {len(blocks)} blocks")
        return DocumentIR(
            doc_id=doc_id,
