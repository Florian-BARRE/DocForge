...
from providers.converter import (
    GOTENBERG_FORMATS,
    NATIVE_PDF_FORMATS,
    GotenbergConverter,
)
...
class S0IngestStage(LoggerClass):
    ...
    async def run(
        ...
        source_hash = self._sha256(file_bytes)
        original_format = self._extract_extension(filename)
        ...
        await self._s3.upload(
            key=original_key,
            data=file_bytes,
            content_type=self._mime_type(original_format),
        )
        ...
        needs_ocr = self._detect_raster_pages(pdf_bytes)
        if needs_ocr:
    ...
    def _sha256(data: bytes) -> str:
        """Compute the SHA-256 hex digest of file bytes (content-address key)."""
    ...
    @staticmethod
    def _count_pages_fast(pdf_bytes: bytes) -> int:
        """Count PDF pages using PyMuPDF without a full parse."""
        try:
            import fitz
    ...
    @staticmethod
    def _detect_raster_pages(pdf_bytes: bytes) -> bool:
        """
    ...
    def _build_implicit_meta(
        *,
        ...
        """Build the file-intrinsic implicit metadata dict (spec §7.3)."""
        return {
            "filename": filename,
    ...
    def _mime_type(extension: str) -> str:
        """Return a best-effort MIME type for an extension."""
        ...
        return _MIME_MAP.get(extension, "application/octet-stream")
