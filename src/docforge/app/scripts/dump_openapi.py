...
#
# Usage (from src/docforge/, the uv project root):
#   uv run python app/scripts/dump_openapi.py > openapi.json
#
# Requires the same env vars as the app (see services/docforge/.env.example). Set
...
# APP_DIR (src/docforge/app) must be on sys.path BEFORE `entrypoint` is imported, so its own
