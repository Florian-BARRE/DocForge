...
# exactly once on creation and never again, and listings expose only safe prefixes. API keys carry
# an optional per-collection capability scope (``permissions``) so root can mint scoped keys.
...
class UserSummary(BaseModel):
    ...
    role: str = Field(..., description="Global role: always 'root' in the keys-only model.")
...
class MeResponse(BaseModel):
    """Response for GET /auth/me — the caller's identity (root, full access)."""

    user: UserSummary = Field(..., description="The authenticated user (the root account).")


class ApiKeyCreateRequest(BaseModel):
    """
    Body for POST /auth/keys — create a new API key (owned by root).

    ``permissions`` is REQUIRED — a created key must declare its scope (no fail-open). A key that
    should reach everything must say so explicitly with an all-collections admin entry, e.g.
    ``{"entries": [{"collection_id": "*", "role": "admin"}]}``. The unscoped full-access sentinel
    (``permissions is None``) is reserved for the static root env key + root login, and is NOT
    creatable through this endpoint. Validated against the capability taxonomy. Shape:
    ...
    permissions: dict = Field(
        ...,
        description=(
            "Per-collection capability scope (REQUIRED — no full-access fallback). For full access "
            "pass an entry with collection_id '*' and role 'admin'. Validated against the taxonomy."
        ),
    )
...
class ApiKeyCreatedResponse(BaseModel):
    ...
    permissions: dict | None = Field(
        default=None, description="The key's per-collection capability scope (None = full access)."
    )
...
class ApiKeyListResponse(BaseModel):
    """Response for GET /auth/keys — the current user's keys (prefixes + scope only)."""
