...
export function IngestionConditionsPanel({
  ...
  // Build a set of field names that already have an LLM target bound in
  // pipeline.metagen.targets — used to flag generated fields missing a prompt.
  const metagenTargetFields = (() => {
    const pipeline = (configState as Record<string, unknown>).pipeline as Record<string, unknown> | undefined
    const targets = (pipeline?.metagen as Record<string, unknown> | undefined)?.targets
    if (!Array.isArray(targets)) return new Set<string>()
  ...
  /** Appends a blank user-defined metadata field row (origin defaults to 'user'). */
  function addField() {
        ...
        <table className="stage-conditions-table meta-schema-table">
          <thead>
            <tr>
              ...
              <th title="Generated by LLM (S5b metagen) — toggles origin between user and generated">Gen.</th>
          ...
          <tbody>
            ...
            {userFields.map((f, idx) => (
              <tr key={idx} className="meta-schema-row-user">
                ...
                {/* Gen. checkbox — toggles origin between 'user' and 'generated'.
                    Disabled for read-only users. Generated fields keep all searchable
                    toggles editable because searchability is a field-identity concern
                    independent of where the value comes from. */}
                <td className="meta-schema-check">
                  <input
                    type="checkbox"
                    checked={f.origin === 'generated'}
                    disabled={!canWrite}
                    title={f.origin === 'generated'
                      ? 'LLM-generated field — value produced by S5b metagen at ingestion'
                      : 'Mark as LLM-generated — value will be produced by S5b metagen'}
                    onChange={e =>
                      canWrite && updateUserField(idx, 'origin', e.target.checked ? 'generated' : 'user')
                    }
                  />
                </td>
                <td>
                  {f.origin === 'generated' ? (
                    <>
                      <span className="tag meta-tag-llm">generated</span>
                      {/* Badge when a generated field has no bound target in metagen.targets */}
                      {!metagenTargetFields.has(f.field_name) && f.field_name && (
                        <span
                          className="tag meta-tag-no-prompt"
                          title="No prompt configured yet — add a target in the MetaGen stage panel"
                        >
                          no prompt
                        </span>
                      )}
            ...
            {systemFields.map(f => (
              <tr key={f.field_name} className="stage-conditions-row-system meta-schema-row-system">
                ...
                {/* System fields are never generated by LLM — empty Gen. cell */}
                <td className="meta-schema-check"></td>
                <td>
