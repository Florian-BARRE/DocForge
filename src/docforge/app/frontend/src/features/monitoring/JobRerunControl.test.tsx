...
// path, the hidden-while-running case (the core repro for "no button to re-run it quickly"), and
// round-4 T10: the relabeled "Re-run (cached)"/"Force (no cache)" buttons + Force's confirm step
// (fires only after Confirm, not on the first click).
...
describe("JobRerunControl", () => {
  it("re-runs the document and navigates to the freshly created job on success", async () => {
    ...
    fireEvent.click(screen.getByText("Re-run (cached)"));
  ...
  it("requires a confirm step before Force actually fires — a raw click only opens the confirm", async () => {
    const accepted: UploadAccepted = { document_id: "doc-1", job_id: "job-3", duplicate: false };
    vi.mocked(reingestDocument).mockResolvedValue(accepted);
    const callsBefore = vi.mocked(reingestDocument).mock.calls.length;
    const onNavigate: Navigate = vi.fn();

    renderWithProviders(<JobRerunControl job={jobFixture({})} collectionId="col-1" onNavigate={onNavigate} />);
    fireEvent.click(screen.getByText("Force (no cache)"));

    expect(vi.mocked(reingestDocument).mock.calls.length).toBe(callsBefore);
    expect(screen.getByText(/Force re-run\?/)).toBeInTheDocument();

    fireEvent.click(screen.getByText("Confirm"));
    await waitFor(() => expect(reingestDocument).toHaveBeenCalledWith("doc-1", { force: true }));
    expect(onNavigate).toHaveBeenCalledWith({ name: "job", collectionId: "col-1", jobId: "job-3" });
  });

  it("lets Back cancel out of the Force confirm without firing", () => {
    // Compare against the call count so far (rather than asserting zero calls outright) — this
    // mock is module-scoped and earlier tests in this file already invoked it.
    const callsBefore = vi.mocked(reingestDocument).mock.calls.length;
    renderWithProviders(
      <JobRerunControl job={jobFixture({})} collectionId="col-1" onNavigate={vi.fn()} />,
    );
    fireEvent.click(screen.getByText("Force (no cache)"));
    fireEvent.click(screen.getByText("Back"));

    expect(vi.mocked(reingestDocument).mock.calls.length).toBe(callsBefore);
    expect(screen.getByText("Force (no cache)")).toBeInTheDocument();
  });
});
