// ====== Code Summary ======
// One store's card — a header (swatch, label, total, "≈ estimated" badge when the backend flags
// it) plus a collapsed-by-default disclosure of that store's per-field byte rows. The card never
// decides what "estimated" means; it only renders the flag it's handed.
...
export function StorageStoreBreakdown({ label, swatchColor, totalBytes, estimated, rows, note }: StorageStoreBreakdownProps) {
  return (
    <div style={{ background: t.color.surface, border: `1px solid ${t.color.line}`, borderRadius: t.radius.l, padding: t.space.l, boxShadow: t.shadow.sm, display: "flex", flexDirection: "column", gap: t.space.m }}>
      <div style={{ display: "flex", alignItems: "center", gap: t.space.s }}>
        ...
        <span style={{ fontFamily: t.font.mono, fontSize: t.font.size.l, color: t.color.text, marginLeft: "auto" }}>{formatBytes(totalBytes)}</span>
