...
def test_insert_once_chains_the_fragment_root_and_validates(
...
def test_insert_twice_suffixes_ids_and_remaps_from_first(
    editor, builder, validator, default_blob
) -> None:
    twice = editor.apply(
        default_blob, [InsertFragment(fragment=_FRAGMENT), InsertFragment(fragment=_FRAGMENT)]
    )
