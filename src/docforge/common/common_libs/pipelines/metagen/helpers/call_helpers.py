# ====== Code Summary ======
# MetagenCallHelpers — static helpers wrapping ONE LLM generate_json call for the metagen scope nodes.
...
class MetagenCallHelpers:
    """
    Static helpers for the metagen scope nodes (cached LLM call + trace conversion).

    No instance state of its own — the LLM chain and provider cache are passed in per call so the
    same logic serves both the chunk-scope and document-scope nodes.
    """
