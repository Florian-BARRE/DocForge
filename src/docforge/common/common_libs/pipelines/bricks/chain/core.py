...
class Chain[T, R](LoggerClass):
    ...
    The chain is built per-collection at assembly and injected into a node as a
    SERVICE (e.g. ``ctx.parser_chain``): the constructor is fully config/instance-driven,
    holds no global state, and depends on no pipeline engine or registry.
