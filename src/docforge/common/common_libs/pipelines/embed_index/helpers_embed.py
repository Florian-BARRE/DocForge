# ====== Code Summary ======
# EmbedIndexEmbedHelpers — the embed-side static helpers shared by the embed_content and embed_fields
# nodes. Runs the embed chain in batches with the load-bearing POSITIONAL CONTRACT (degraded batches
# emit same-length None placeholders so vectors stay 1:1 with their chunks), and converts each chain
# outcome into a domain ChainTrace for lineage. Pure over an injected chain.
...
class EmbedIndexEmbedHelpers:
    ...
    logger = loggerplusplus.bind(identifier="EmbedIndexEmbedHelpers")

    def __new__(cls, *args: object, **kwargs: object) -> None:
        """Block instantiation — this is a static-only helper class."""
        raise TypeError("EmbedIndexEmbedHelpers is a static-only class and cannot be instantiated.")
    ...
    async def embed_texts(
        ...
        Args:
            chain (Chain): The ordered embed chain (a service injected into the node).
...
__all__ = ["EmbedIndexEmbedHelpers"]
