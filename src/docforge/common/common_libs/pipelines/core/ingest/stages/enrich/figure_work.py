# ====== Code Summary ======
# FigureWork - the mutable per-figure working state threaded across the enrich stage's per-capability
# step outputs. Where the legacy FigureEnricher processed ONE figure end-to-end (classify -> OCR ->
# VLM -> chart), the native enrich stage runs ONE capability over ALL figures per step. The routing
# DECISION (taken once, at classify time) and the per-figure intermediate artefacts (crop bytes/hash,
# kind, OCR text, VLM description + raw structured output, data table, accumulated chain traces) must
# therefore outlive a single step: they live on a FigureWork seeded by the classify step and carried
# (as a typed list, NOT ctx.aux) through each downstream step's Input/Output. The IR builder reads
# :meth:`enrichment` + :meth:`traces` to rebuild each FIGURE block.
...
class FigureWork:
    ...
    builder reads :meth:`enrichment` + :meth:`traces` to rebuild each FIGURE block.
