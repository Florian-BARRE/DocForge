# ====== Code Summary ======
# The chunk-scope step's context — narrows ``input`` to this step's typed input and exposes the LLM
...
from .io import IngestStageMetagenStepChunkScopeInput


class IngestStageMetagenStepChunkScopeContext(IngestStageMetagenStepContextBase):
    """Context for the chunk-scope step (typed input + the LLM chain + the provider cache)."""

    @property
    def input(self) -> IngestStageMetagenStepChunkScopeInput:
...
__all__ = ["IngestStageMetagenStepChunkScopeContext"]
