# ====== Code Summary ======
# IngestStageEmbedIndexResult — the typed output payload of the embed_index stage (the former
# S6Result). It carries the per-run counts (chunks embedded, points upserted to Qdrant, chunk rows
# persisted to Postgres), the target collection, the number of per-field named vectors materialised,
# and the per-batch embed chain traces the orchestrator flushes onto the document IR for lineage.
...
class IngestStageEmbedIndexResult:
    """
    ...
    Attributes:
        ...
        collection_name (str): Qdrant collection the points were upserted into.
...
__all__ = ["IngestStageEmbedIndexResult"]
