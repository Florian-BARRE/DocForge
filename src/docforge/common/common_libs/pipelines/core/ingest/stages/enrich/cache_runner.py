...
# ProviderCallCache. Each helper returns a (result, trace, was_hit) tuple so the caller (the classify
# / OCR steps) never needs to know the cache-key mechanics. Cache-key resolution is delegated to
# CallKeyHelpers; the VLM capability lives in VlmRunner (it resolves a chart-to-data schema from the
# concrete provider type). The cache keys are content-based, so running these per capability over all
# figures yields the same hit/miss pattern as the legacy per-figure path.
...
from common_libs.providers import ClassificationResult, OcrHint, OcrResult
...
class CacheRunner:
    ...
    1. Derive a provider-call fingerprint (via CallKeyHelpers) from the capability name, provider
       id/version, call params, and the crop content-hash.
    ...
    async def run_classify(
        ...
        The classification result is keyed by ``crop_hash`` so a repeated logo or header runs the
        classifier model exactly once across every document.
        ...
        resolved = CallKeyHelpers.resolve(classifier_chain, "classifier", "classifier", {}, crop_hash)
        if resolved is None:
    ...
    async def run_ocr(
        ...
        A cache hit emits a synthetic trace whose ``final_provider`` is ``"provider_cache"`` so
        block-level lineage shows "served from provider-call cache".
