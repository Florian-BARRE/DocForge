# ====== Code Summary ======
# MetagenPromptHelpers — static, I/O-free helpers shared by the metagen steps: partition targets by
# scope, build the per-field rule block, the chunk-scope and document-scope prompts, the document
# digest, and a coarse per-call cost estimate for the budget gate. Extracted so every step stays
...
class MetagenPromptHelpers:
    """
    Static helpers for the metagen steps (target partitioning, prompts, cost estimate).
