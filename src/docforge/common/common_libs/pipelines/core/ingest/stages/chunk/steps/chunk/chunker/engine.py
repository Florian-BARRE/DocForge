# ====== Code Summary ======
# StructureAwareChunker — heading-hierarchy-aware chunking engine. Wires HeadingWalker (skeleton +
...
class StructureAwareChunker(LoggerClass):
    """
    Heading-hierarchy-aware chunker.
        ...
        *,
        ...
        Args:
            ...
            reinject_breadcrumb (bool): Record the section breadcrumb on each chunk so split
                sub-chunks remain section-aware (consumed by contextualize for embed_text).
        ...
        for rule in heading_rules or []:
            try:
                ...
                self.logger.warning(f"Chunk: skipping invalid heading rule {rule!r}")
        self._config_hash = self._compute_config_hash(heading_rules or [])

    @property
    def config_hash(self) -> str:
        """The deterministic blake2b hash of the chunking configuration."""
        return self._config_hash

    def params_for_fingerprint(self) -> dict[str, Any]:
        """
        Return chunking parameters for the chunk Merkle fingerprint.
    ...
    async def run(self, ir: DocumentIR) -> S4Result:
        ...
        Args:
            ir (DocumentIR): Enriched DocumentIR from the enrich stage.

        Returns:
            S4Result: Chunks in reading order, each tagged with its section breadcrumb.
        """
        self.logger.info(
            f"Chunk started: doc_id={ir.doc_id} blocks={len(ir.blocks)} "
        ...
        # 4. Cross-reference linking — best-effort, mutates prov in place
        ...
        self.logger.info(
            f"Chunk done: doc_id={ir.doc_id} chunks={len(chunks)} "
