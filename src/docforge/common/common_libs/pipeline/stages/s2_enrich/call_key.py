...
class CallKeyHelpers:
    ...
    The first provider drives the cache key because it is the one that *would* run on a
    cache miss; escalation to later providers only happens when the first one fails the gate.
    """
    ...
    def resolve(
        ...
        Returns:
            tuple[Any, str, str, str] | None: ``(first_provider, provider_id,
                provider_version, call_fp)`` or None when the chain has no providers.
        """
