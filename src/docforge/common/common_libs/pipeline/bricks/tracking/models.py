...
# level REUSES the existing pipeline.bricks.chain.ChainAttempt type (no reinvention) so the per-
# provider escalation lineage already produced by Chain.call flows straight into the trace.
...
from common_libs.pipeline.bricks.chain import ChainAttempt
...
class StageTrace:
    ...
    Attributes:
        ...
        cache_hit (bool): True when the stage was served from the node cache (PR-3+).
