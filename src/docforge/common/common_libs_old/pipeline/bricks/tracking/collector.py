...
# A trace is bound to a PipelineContext (stored under ``ctx.aux["execution_trace"]``) so any
...
# Key under which the active ExecutionTrace lives on ``PipelineContext.aux``.
...
class ExecutionTrace:
    ...
    @classmethod
    def for_context(cls, ctx: "PipelineContext") -> "ExecutionTrace":
        """
        Return the trace bound to ``ctx``, creating and attaching one on first access.

        Args:
            ctx (PipelineContext): The active pipeline context.
