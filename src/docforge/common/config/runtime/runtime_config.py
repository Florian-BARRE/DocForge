...
class RUNTIME_CONFIG(EnvConfigLoader):
    ...
    # ───── Paths & dirs ─────
    # Root of the SHARED tree (src/common/). This file lives at
    # src/common/config/runtime/runtime_config.py → parent×3 = src/common/.
    ...
    #   `from common_libs.<bucket> import …`  and  `from config import RUNTIME_CONFIG`.
    # The DEDICATED libs (backend/libs or worker/libs) are NOT here — each app's
    # entrypoint (backend/entrypoint.py, worker/arq_worker.py) adds its own dir to
    # sys.path before importing this module, so `from libs.<x> import …` resolves to
    # the running app's dedicated tree.
    ...
    # NOTE: the compiled frontend path is backend-specific and no longer lives in the
    # shared tree (the frontend moved to src/backend/frontend/). It is computed directly
    # in src/backend/entrypoint.py, which knows its own location — not here.
