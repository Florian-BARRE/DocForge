# ====== Code Summary ======
# Response models for the capabilities resource, mirrored field-for-field from the DocForge backend
# router models (app/backend/libs/capabilities/models.py): the deployment self-description served by
# the public GET /capabilities — running version, auth state, derived GPU presence, the infra stores +
# optional sidecars (reachability + what each unlocks) and the available pipeline kinds per family.
...
class ServiceInfo(BaseModel):
    ...
    Attributes:
        ...
        reachable (bool): Sidecar: its short-cached ``/health`` probe currently passes. Store:
            configured (declared, not live-probed — see ``detail``).
        device (str | None): The compute device the sidecar reports at ``/health`` when it exposes one;
            ``None`` when unknown or not applicable (stores).
        provides (list[str]): The capability ids (``"family:kind"``) this service enables — empty for
            infra stores.
        detail (str | None): Optional short status note.
    """
...
class CapabilityMatrix(BaseModel):
    ...
    is reachable. ``metagen`` delegates to the ``llm`` family, so it is legitimately empty.
...
class CapabilitiesResponse(BaseModel):
    ...
    Attributes:
        version (str): The running app/image version (``FASTAPI_APP_VERSION`` / ``DOCFORGE_TAG``).
        ...
        services (list[ServiceInfo]): The infra stores + optional sidecars.
