# ====== Code Summary ======
# Cheap, cached probe for whether a CUDA GPU is visible to this container. dots.ocr's VLM requires
# CUDA: on a host with no GPU it would fail at model load — so /health consults `cuda_available()`
# (gated by DOTS_OCR_REQUIRE_GPU) and reports UNHEALTHY up front, instead of advertising a readiness the
# container cannot honor and then failing mid-parse. This mirrors mineru_server's GpuFeatures.
#
...
# a POSITIVE GPU signal is required here, because running the VLM without CUDA is a hard failure.
...
class GpuFeatures:
    ...
    def _probe(cls) -> bool:
        ...
        cls.logger.error(
            f"No CUDA GPU detected — dots.ocr's VLM requires one; reporting UNHEALTHY."
        )
        return False
