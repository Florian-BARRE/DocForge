...
IR = DocumentIR(doc_id="d1", source_hash="h", n_pages=1, blocks=[
    _fig_block("f_scan", b"EASY-SCAN", bbox=(0.0, 0.0, 1.0, 1.0), order=0),   # heuristic: full page
    _fig_block("f_hard", b"HARD-SCAN", bbox=(0.0, 0.0, 1.0, 1.0), order=1),   # full page + hard OCR
...
def _vlm(node_id: str, prompt: str, extract_table: bool = False) -> dict:
    return {"node_type": "action", "id": node_id, "family": "vlm", "kind": "test_enrich_stage_vlm",
            "config": {"system_prompt": prompt, "extract_table": extract_table}}


BLOB = {
    "node_type": "group", "id": "enrich_stage",
    "nodes": [
        ...
        {"node_type": "foreach", "id": "per_figure",
         ...
         "item_field": "figure", "max_concurrency": 3,
         "body": {
             "node_type": "group", "id": "treat",
             "nodes": [
                 {"node_type": "action", "id": "clf", "family": "enrich",
                  "kind": "test_enrich_stage_classify", "config": CLF_CONFIG},
                 {"node_type": "action", "id": "ocr_cheap", "family": "ocr",
                  "kind": "test_enrich_stage_ocr_cheap", "config": {}},
                 {"node_type": "action", "id": "ocr_robust", "family": "ocr",
                  "kind": "test_enrich_stage_ocr_robust", "config": {}},
             ...
             "transitions": [
                 {"from_node_id": "clf", "to_node_id": "ocr_cheap",
                  "condition": {"kind": "when_equals", "field": "kind", "equals": "scanned_text"}},
                 {"from_node_id": "clf", "to_node_id": "vlm_photo",
                  "condition": {"kind": "when_equals", "field": "kind", "equals": "photo"}},
                 {"from_node_id": "clf", "to_node_id": "vlm_chart",
                  "condition": {"kind": "when_equals", "field": "kind", "equals": "chart"}},
                 {"from_node_id": "clf", "to_node_id": "vlm_diag",
                  "condition": {"kind": "when_equals", "field": "kind", "equals": "diagram"}},
                 {"from_node_id": "clf", "to_node_id": "skip",
                  "condition": {"kind": "when_equals", "field": "kind", "equals": "decorative"}},
                 {"from_node_id": "ocr_cheap", "to_node_id": "ocr_robust",
             ...
             "bindings": {
                 ...
                 "ocr_cheap": {"figure": {"source": "node", "node_id": "clf", "field_name": "figure"}},
                 "ocr_robust": {"figure": {"source": "node", "node_id": "clf", "field_name": "figure"}},
                 "vlm_scan": {"figure": {"source": "first", "candidates": [
