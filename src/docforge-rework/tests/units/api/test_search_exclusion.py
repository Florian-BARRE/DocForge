...
EVERY query, unbypassable from the router: an ``enabled == true`` match (drops disabled chunks'
points) and a ``must_not`` over the collection's disabled document ids (drops disabled documents'
points), the latter fed by a bounded Postgres lookup. Qdrant + Postgres are fully mocked — the test
proves the FILTER shape, which is what makes Qdrant hide those points at query time."""
...
async def test_hybrid_injects_enabled_match_and_disabled_doc_exclusion(monkeypatch) -> None:
    """A disabled document in the collection → an `enabled==true` must + a document_id must_not."""
    ...
    # 4. The query carries the user match AND the enabled-only match; the disabled doc is excluded.
    kwargs = hybrid.await_args.kwargs
    assert Match(field="topic", value="ai") in kwargs["conditions"]
    assert Match(field="enabled", value=True) in kwargs["conditions"]
...
async def test_hybrid_has_no_exclusion_when_no_disabled_docs(monkeypatch) -> None:
    """No disabled documents → still filters `enabled==true`, but with an empty must_not set."""
    ...
    # 3. The enabled-only guard is always present; there is nothing to exclude.
    kwargs = hybrid.await_args.kwargs
    assert kwargs["conditions"] == [Match(field="enabled", value=True)]
    assert kwargs["exclusions"] == []
