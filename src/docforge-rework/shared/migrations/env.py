...
# complete, and reads POSTGRES_DSN from the environment (services/docforge-rework/.env in local
...
# ─── Local dev: load the stack's .env (no-op in containers, vars already set) ───
safe_load_envs(
    path=pathlib.Path(__file__).resolve().parents[4] / "services" / "docforge-rework",
    verbose=False,
)
...
def _database_url() -> str:
    """The sync driver URL (alembic runs sync; the apps use asyncpg at runtime)."""
...
def run_migrations_online() -> None:
    """Run migrations against the live database (the normal mode)."""
