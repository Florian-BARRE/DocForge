...
# metagen) so every consumer always reads the nearest enabled producer. Because the assembler
# recomputes wiring from the enabled set, the compiler can toggle any stage and still emit a blob
# that builds: the "no doubt, coherent end to end" guarantee lives here. default_blob() is just
# assemble(default_state()).
...
class _Segment:
    ...
    def __init__(self, head: str, tail: str, nodes: list, transitions: list) -> None:
        self.head = head
        self.tail = tail
        self.nodes = nodes
...
class IngestAssembler:
    ...
    def assemble(cls, state: PipelineState) -> GroupNodeBlob:
        ...
        # 2. Flatten nodes + internal transitions, then chain consecutive segments tail → head.
        nodes: list = []
        ...
        for previous, current in zip(segments, segments[1:], strict=False):
            transitions.append(Transition(from_node_id=previous.tail, to_node_id=current.head))
    ...
    def __single(cls, node_id: str, family: str, kind: str, config: dict) -> _Segment:
        ...
        return _Segment(head=node_id, tail=node_id, nodes=[node], transitions=[])
    ...
    def __intake(cls, state: PipelineState) -> _Segment:
        ...
        return _Segment(head="probe", tail="address", nodes=nodes, transitions=chain)
    ...
    def __enrich(cls, state: PipelineState) -> _Segment:
        ...
        return _Segment(head="extract", tail="apply", nodes=nodes, transitions=transitions)
    ...
    def __contextualize(cls, state: PipelineState) -> _Segment:
        ...
        chain = [Transition(from_node_id=a, to_node_id=b) for a, b in zip(ids, ids[1:], strict=False)]
        return _Segment(head=ids[0], tail=ids[-1], nodes=nodes, transitions=chain)
    ...
    def __bindings(cls, state: PipelineState) -> dict[str, dict]:
        ...
        # 1. The spine anchor points that depend on which stages are on.
        ir_pre_enrich = ("figures", "ir") if state.render_on else ("parse", "ir")
        ir_final = ("apply", "ir") if state.enrich_on else ir_pre_enrich
        ctx_ids = cls.contextualize_ids(state)
        chunks_pre_meta = (ctx_ids[-1], "chunks") if ctx_ids else ("chunk", "chunks")
        chunks_final = ("meta_chunk", "chunks") if state.metachunk_on else chunks_pre_meta
        ...
        if state.enrich_on:
            bindings["extract"] = {"ir": cls.__from(ir_pre_enrich)}
            bindings["apply"] = {"ir": cls.__from(ir_pre_enrich),
                                 "entries": FromNode(node_id="per_figure", field_name="items")}
        bindings["chunk"] = {"ir": cls.__from(ir_final)}
        cls.__bind_stack(bindings, state, ctx_ids)
        if state.metachunk_on:
            bindings["meta_chunk"] = {"chunks": cls.__from(chunks_pre_meta),
                                      "contract": FromRunInput(field_name=_CONTRACT)}
        if state.metadoc_on:
            bindings["meta_doc"] = {"chunks": cls.__from(chunks_final),
                                    "contract": FromRunInput(field_name=_CONTRACT)}
        if state.embed_on:
            bindings["embed"] = {"chunks": cls.__from(chunks_final),
                                 "contract": FromRunInput(field_name=_CONTRACT)}
    ...
    def __bind_stack(cls, bindings: dict[str, dict], state: PipelineState, ctx_ids: list[str]) -> None:
        ...
        previous = ("chunk", "chunks")
        for ctx_id, method in zip(ctx_ids, state.stack, strict=True):
            slots: dict = {"chunks": cls.__from(previous)}
            ...
            previous = (ctx_id, "chunks")

    @classmethod
    def __bundle_bindings(cls, state: PipelineState, ir_final: tuple, chunks_final: tuple) -> dict:
        ...
        slots: dict = {"ingest": FromNode(node_id="address", field_name="ingest"),
                       "ir": cls.__from(ir_final),
                       "chunks": cls.__from(chunks_final)}
        if state.render_on:
    ...
    @staticmethod
    def __from(source: tuple) -> FromNode:
        """A FromNode from a (node_id, field_name) spine anchor."""
        return FromNode(node_id=source[0], field_name=source[1])
