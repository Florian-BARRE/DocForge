# ====== Code Summary ======
# Cheap, MinerU-free validation that a raw request body actually decodes as a PDF (the media type the
...
# reaching MinerU and surfacing a raw exception string at HTTP 500 — which both mis-signals a server
# fault and leaks internal detail. Genuine server faults still fall through to `@auto_handle_errors`
# -> HTTP 500.
#
# pypdfium2 is AVX-free, lightweight, and a base dependency (NOT part of the heavy MinerU/torch extra),
# so importing this module never touches MinerU/CUDA — it stays unit-testable on this CPU-only VM.
