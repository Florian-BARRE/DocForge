# ====== Code Summary ======
# RequestBodyGuard — bounds how many bytes the raw-body /parse route will buffer. The route reads the
# WHOLE body into memory (the body IS the PDF), so an uncapped request is an OOM vector against a
# container whose memory ceiling exists for model spikes, not attacker payloads. The guard refuses a
# declared Content-Length over the cap before reading a byte, then caps the streamed read so a lying or
# absent length cannot bypass it.
