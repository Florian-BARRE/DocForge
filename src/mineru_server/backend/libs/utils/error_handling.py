...
# so internal details are never exposed to callers. Mirrors src/paddle_server's identical helper —
# this is an internal model-host with no debug-mode toggle, so there is no branch that would
# surface the traceback in the body.
