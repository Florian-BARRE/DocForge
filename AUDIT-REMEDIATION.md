...
- **2026-09-06 — Vague AL / 0.14.46 (524 IMPLÉMENTÉ + release-testé — l'unique item restant est clos pour de vrai)** : les deux workflows release fusionnés en UN seul top-level `release-sdk.yml` (nom de fichier conservé pour l'OIDC PyPI) : le gate réutilisable tourne **une seule fois** par tag, puis version-guard → images (matrice 8, tags de version seuls) → retag-latest (`:latest` atomique) → publish PyPI → security-scan ; jobs images `if:`-gatés sur `v*` (le `sdk-v*` reste SDK-only) ; `release-images.yml` supprimé. **PAS** l'approche 0.14.5 (qui enveloppait release-sdk.yml en reusable → casse l'OIDC → revert 0.14.6) : les jobs images sont absorbés DANS le top-level, l'OIDC est préservé. **Validé end-to-end sur la release 0.14.46** : gate unique ✓, 8 images ✓, retag :latest atomique ✓, publish PyPI ✓, scan ✓. (Le publish a heurté un reset réseau Sigstore/Rekor transitoire au 1er essai — sans rapport avec la fusion — et a réussi au re-run.) → **524 réellement corrigé** ; la topologie double-workflow (et le gate en double) n'existe plus. **AUDIT 247/247 sans aucun item déféré.**

- **2026-09-06 — Vague AK / no-release (532 + 583 : documenté → IMPLÉMENTÉ pour de vrai)** : les deux items infra précédemment clos « par décision documentée » sont désormais réellement corrigés. **583** : les 9 fichiers compose renommés `compose/*.yml` → `compose/compose.*.yml` (via `git mv`, historique préservé) pour matcher le glob Dependabot `(docker-)?compose(\.[\w-]+)?\.ya?ml` → la couverture des ~11 image-pins de service est ENFIN active ; toutes les références mises à jour (root `docker-compose.yml`, Makefile, CLAUDE.md, README, compose/README.md, en-têtes des 9 fichiers, docs/**, .env.example, services/**, Dockerfiles, dependabot.yml) ; **bug latent Makefile corrigé** (`config-check-all` calculait `$$scenario.yml` depuis la variable de boucle nue → `compose.$$scenario.yml`). Validé : 16/16 scénarios×add-ons `config -q` OK. **532** : fenêtre de release partielle éliminée — le job matrice `images` ne pousse plus QUE les tags de version immuables ; un nouveau job `retag-latest` (`needs: images`) déplace tous les `:latest*` via `docker buildx imagetools create` UNE fois que toutes les branches ont réussi → `:latest` ne bouge plus jamais partiellement. YAML validé. **NB 524** (double-gate par tag) : SEUL item non-code-fixé, à raison. L'orchestrateur unifié tenté en 0.14.5 a été **reverté en 0.14.6** (PyPI Trusted Publishing matche le sujet OIDC sur le workflow TOP-LEVEL → envelopper `release-sdk.yml` en reusable fait rejeter l'upload). Le fix SÛR = absorber les jobs images/retag DANS le top-level `release-sdk.yml` (garder ce nom de fichier PyPI-trusted, gater les jobs images sur `v*`), mais c'est **intestable sans une vraie release** → déféré à un changement dédié testé plutôt que poussé à l'aveugle sur un pipeline à 15 releases vertes. Raison documentée dans `ci.yml`. Compromis délibéré (quelques minutes CI vs topologie known-good).
